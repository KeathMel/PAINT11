...
class PaintApp:
    ...
    def _handle_mouse_move(self, event):
        ...
        # Move floating selection with cursor
        if self.floating_selection:
            self.floating_pos = (cx, cy)
            # Keep the selection outline in sync so marching ants follow
            w, h = self.floating_selection.get_size()
            self.selection = (cx, cy, w, h)
            self.selection_active = True
            return
