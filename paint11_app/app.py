...
class PaintApp:
    ...
    def __init__(self, open_path=None):
        ...
        self.floating_selection = None  # extracted content being moved
        self.floating_pos = None  # current position of float (x,y)
        self.float_dragging = False  # True while actively dragging the float
        self.float_grab_offset = (0, 0)  # where inside the float we grabbed
    ...
    def _handle_mouse_down(self, event):
        ...
        # Middle-mouse pan: start tracking pan on middle-button press
        if btn == 2:
            self.panning = True
        ...
        if self.canvas_area.collidepoint(mx, my):
            ...
            if self.tool == "select_rect":
                # If something is floating, check if we clicked outside to drop it
                if self.floating_selection and self.floating_pos:
                    fx, fy = self.floating_pos
                    w, h = self.floating_selection.get_size()
                    if not (fx <= cx <= fx+w and fy <= cy <= fy+h):
                        # Clicked outside — drop it here
                        self.canvas.surface.blit(self.floating_selection, (fx, fy))
                        self.selection = (fx, fy, w, h)
                        self.selection_active = True
                        self.floating_selection = None
                        self.floating_pos = None
                    else:
                        # Clicked inside — grab it again to keep dragging
                        self.float_grab_offset = (cx - fx, cy - fy)
                        self.float_dragging = True
                    return
                
                # Check if clicking inside existing selection to float it
                if self.selection_active and self.selection:
                    ...
                    if sx <= cx <= sx+sw and sy <= cy <= sy+sh:
                        ...
                        if sw2 > 0 and sh2 > 0:
                            # Extract the selection content and erase from canvas
                            self.floating_selection = self.canvas.surface.subsurface(
                                pygame.Rect(sx, sy, sw2, sh2)).copy()
                            self.canvas.surface.fill((0, 0, 0, 0), pygame.Rect(sx, sy, sw2, sh2))
                            self.floating_pos = (sx, sy)
                            # Remember where inside the selection we grabbed it,
                            # so it doesn't snap its corner to the cursor
                            self.float_grab_offset = (cx - sx, cy - sy)
                            self.float_dragging = True
    ...
    def _handle_mouse_up(self, event):
        ...
        # Middle-mouse pan: stop on release
        if self.panning:
            self.panning = False
            self.pan_start = None
            return

        # Stop dragging the float, but keep it floating
    ...
    def _handle_mouse_move(self, event):
        ...
        # Middle-mouse pan: drag to scroll the view
        ...
        # Move floating selection with cursor — only while dragging
        if self.floating_selection and self.float_dragging:
            gx, gy = self.float_grab_offset
            self.floating_pos = (cx - gx, cy - gy)
            # Keep the selection outline in sync so marching ants follow
            w, h = self.floating_selection.get_size()
            self.selection = (cx - gx, cy - gy, w, h)
            self.selection_active = True
            return
    ...
    def _handle_wheel(self, event):
        ...
        if mods & pygame.KMOD_CTRL:
            # Zoom to cursor: keep the canvas point under the cursor
            # in the same screen position before and after zoom
            ca = self.canvas_area
            if ca.collidepoint(mx, my):
                cx_before = (mx - ca.x - self.scroll_x) / self.zoom
                cy_before = (my - ca.y - self.scroll_y) / self.zoom
                ...
                cx_after = (mx - ca.x - self.scroll_x) / self.zoom
                cy_after = (my - ca.y - self.scroll_y) / self.zoom
                self.scroll_x += int((cx_after - cx_before) * self.zoom)
                self.scroll_y += int((cy_after - cy_before) * self.zoom)
            else:
                # Outside canvas, just zoom normally
                if event.y > 0:
                    self.zoom = min(16.0, self.zoom * 1.1)
                else:
                    self.zoom = max(0.05, self.zoom / 1.1)
    ...
    def render(self):
        ...
        # Floating selection: render it on top following the cursor
        if self.floating_selection and self.floating_pos:
            fx, fy = self.floating_pos
            fx_s, fy_s = self.canvas_to_screen(fx, fy)
            scaled_float = pygame.transform.scale(self.floating_selection, 
                                                   (int(self.floating_selection.get_width() * self.zoom),
                                                    int(self.floating_selection.get_height() * self.zoom)))
            surf.blit(scaled_float, (fx_s, fy_s))
