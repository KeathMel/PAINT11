...
class PaintApp:
    ...
    def _handle_mouse_up(self, event):
        ...
        if self.selection_moving:
            # Commit the move to the canvas: do one final blit to make the
            # moved content permanent. Without this, the move only existed
            # as a visual during the drag (in render()) but the actual
            # canvas.surface was never updated.
            if self.selection_copy and getattr(self, "_move_base", None) is not None:
                sx, sy, sw, sh = self.selection
                surface = self.canvas.surface
                surface.fill((0, 0, 0, 0))
                surface.blit(self._move_base, (0, 0))
                surface.blit(self.selection_copy, (sx, sy))
